...
var startTheQuadratic = d3.select("#canvasQuadratic").selectAll("svg")
...
function startQuadratic() {
  ...
  visQuadratic.selectAll("circle.control_Quadratic")
    .data(function(d) {
      return pointsQuadratic.slice(0, d)
    })
...
function updateQuadratic(getCurveQuadraticData, getDotQuadraticData) {
  var interpolationQuadratic = visQuadratic.selectAll("g")
    .data(function(d) {
      return getLevelsForlineQuadratic(d, t);
    });
...
function x_Quadratic(d) {
  return d.x;
}

function y_Quadratic(d) {
  return d.y;
}
